...
class SpaceShip(pygame.sprite.Sprite):
	def _init_(self,path,x_pos,y_pos):
		...
		self.charged = pygame.image.load('spaceship.png')

		self.image = self.uncharged
		self.rect = self.image.get_rect(center = (x_pos,y_pos))
		self.shield_sureface = pygame.image.load('shield.png')
		self.health = 5
	...
	def display_health(self):
		for index,shield in enumerate(range(self.health)):
			screen.blit(self.shield_sureface,(10 + index * 40, 10))
...
class Meteor(pygame.sprite.Sprite):
	def _init_(self,path,x_pos,y_pos,xspeed,y_speed):
		super()._init_()
...
class Laser(pygame.sprite.Sprite):
	def _init_(self,path,pos,speed):
		super()._init_()
		self.image = pygame.image.load(path)
...
scapeship_group = pygame.sprite.GroupSingle()
scapeship_group.add(spaceship)
...
while True: # Game loop
	for event in pygame.event.get(): # Check for events / Player input
		...
		if event.type == METEOR_EVENT:
			...
			random_y_pos = ramdom.randrange(-500,-50)
			random_x_speed = ramdom.randrange(-1,1)
			random_y_speed = ramdom.randrange(4,10)
